      ...
      <div className="bg-[#f3f0eb] rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.2)] border border-white/80 max-w-xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 sm:p-6">
          ...
          {loading ? (
            ...
            <p className="text-sm text-gray-500 text-center py-8">Todavía no hay ventas registradas en este periodo</p>
          ) : (
            <div className="space-y-2">
              {topEmployees.map((row, idx) => (
                <div key={row.employee?._id || idx} className="flex items-center gap-3 bg-white/80 rounded-2xl border border-white/80 p-3">
                  ...
                  {row.employee?.personalInfo?.image ? (
                    ...
                    <div className="w-9 h-9 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 text-xs font-display font-bold shrink-0">
                      {employeeName(row.employee).split(' ').map((p) => p[0]).slice(0, 2).join('').toUpperCase()}
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <p className="font-display font-semibold text-gray-900 text-sm truncate">{employeeName(row.employee)}</p>
                    <p className="text-xs text-gray-500 truncate">{row.orderCount} pedidos entregados</p>
                  </div>
                  <p className="font-display font-bold text-red-600 text-sm shrink-0">${Number(row.totalSales || 0).toFixed(2)}</p>
                </div>
              ))}
            </div>
          )}
