    ...
    <div className="space-y-3">
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="flex-1 relative">
          <FAIcon icon="magnifying-glass" size="xs" className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Buscar..."
            className="w-full pl-8 pr-3 py-2 bg-white border border-white/80 rounded-xl text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500/30"
          />
        </div>
        {categories.length > 0 && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        ...
        {paginatedItems.length === 0 && (
          <p className="col-span-full text-center text-xs text-gray-400 py-4">Sin resultados</p>
        )}
      </div>

      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button type="button" onClick={prev} disabled={page === 1} className="w-7 h-7 flex items-center justify-center rounded-lg bg-white border border-white/80 disabled:opacity-40">
            <FAIcon icon="chevron-left" size="xs" />
          </button>
          <span className="text-xs text-gray-500">{page}/{totalPages}</span>
          <button type="button" onClick={next} disabled={page === totalPages} className="w-7 h-7 flex items-center justify-center rounded-lg bg-white border border-white/80 disabled:opacity-40">
