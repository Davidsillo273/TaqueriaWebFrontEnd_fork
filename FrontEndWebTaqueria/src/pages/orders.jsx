...
function OrdersContent() {
  ...
  // Filtrado de pedidos según estado real del backend
  ...
  // Abre la confirmación para eliminar un pedido
  ...
  // Actualiza el estado del pedido y muestra toast
          ...
          <div className="p-4 sm:p-6 lg:p-8 flex flex-col lg:flex-row gap-6">
            <div className="flex-1">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Gestión de Pedidos</h1>
                ...
                <div className="bg-gray-200/60 p-1 rounded-xl flex gap-1 border border-gray-200">
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-colors ${tabActiva === 'pendientes' ? 'bg-white text-red-600 shadow-sm' : 'bg-transparent text-gray-500 hover:text-gray-700'}`}
                  >
                    Pendientes ({pendientes.length})
                  </button>
                  <button
                    onClick={() => setTabActiva('listos')}
                    className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-colors ${tabActiva === 'listos' ? 'bg-white text-red-600 shadow-sm' : 'bg-transparent text-gray-500 hover:text-gray-700'}`}
                  >
              ...
              {loading ? (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
                    <div className="bg-white p-4 sm:p-5 rounded-xl border-l-4 border-l-red-600 shadow-sm">
                      <span className="text-xs font-semibold text-gray-500 uppercase">Total Pedidos Hoy</span>
                      <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-1">{totalPedidosHoy}</p>
                    </div>
                    <div className="bg-white p-4 sm:p-5 rounded-xl border-l-4 border-l-red-600 shadow-sm">
                      <span className="text-xs font-semibold text-gray-500 uppercase">Tiempo Promedio</span>
                      <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-1">14m 30s</p>
                    </div>
                    <div className="bg-white p-4 sm:p-5 rounded-xl border-l-4 border-l-red-600 shadow-sm">
                      <span className="text-xs font-semibold text-gray-500 uppercase">Órdenes en Cocina</span>
                      <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-1">{ordenesEnCocina}</p>
                    </div>
                  </div>

                  <h3 className="text-lg font-bold text-gray-800 mb-4 border-l-4 border-l-red-600 pl-2 uppercase">Órdenes Prioritarias</h3>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    {pendientes.map((p) => (
                      <div key={p._id} className="bg-white p-4 sm:p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between min-h-[200px] relative">
                        <button
                          onClick={() => handleRequestDelete(p._id)}
                          className="absolute top-4 right-4 text-gray-400 hover:text-red-600 p-1 transition-colors"
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-1 pr-6">
                            <span className="font-bold text-sm text-gray-900">#{p._id.slice(-4).toUpperCase()}</span>
                            <span className="font-bold text-sm text-gray-900">${Number(p.total).toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between text-xs text-gray-500 font-medium mb-3">
                            ...
                            <span className={p.status === 'cooking' ? 'text-orange-600 font-bold' : ''}><FAIcon icon="clock" size="xs" /> {p.status === 'cooking' ? 'Cocinando' : 'En cola'}</span>
                          </div>
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-lg flex items-center justify-center gap-2 transition-colors"
                        >
            ...
            {/* Panel lateral de resumen (oculto en móvil si no hay espacio, se mantiene en lg) */}
            <div className="w-full lg:w-72 flex flex-col gap-6 mt-6 lg:mt-0">
              <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                <h3 className="text-sm font-bold text-gray-800 mb-4">Resumen del Día</h3>
                <div className="h-28 flex items-end justify-between gap-2 px-2 mb-4 border-b border-gray-100 pb-2">
                  ...
                  <div className="w-full bg-red-600 h-24 rounded-t"></div>
                  <div className="w-full bg-gray-200 h-28 rounded-t"></div>
                </div>
                <div className="flex justify-between text-xs text-gray-400 font-bold px-1 mb-3">
